...
ParticleGenerator::ParticleGenerator(ParticleSystem* pS, float frequency, Vector3D pos, Vector3D dir, float mass, float avgVel, float velVariance, float damp, double lifeSpan, float maxDistance, Vector4 color, PxReal size)
...
void ParticleGenerator::AddParticle()
{
	...
	pS->AddParticle(pos, dir * randomVel, mass, damp, color, size, lifeSpan, maxDistance);
}
